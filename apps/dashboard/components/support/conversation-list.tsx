...
export type KindFilter = 'all' | 'support' | 'internal';
        ...
        <div className="flex gap-1 border-b border-border-subtle px-3 py-2">
          {kindLabels.map((k) => (
            <button
              ...
              className={`rounded-md px-2.5 py-1 text-xs font-medium transition-colors ${kindFilter === k
                  ? 'bg-bg-active text-accent'
                  : 'text-text-secondary hover:bg-bg-hover hover:text-text-primary'
                }`}
            >
              {k === 'all' ? t('all') : k === 'support' ? t('kindSupport') : t('kindInternal')}
            </button>
          ))}
        </div>
      )}
      ...
      <div className="flex gap-1 border-b border-border-subtle px-3 py-2">
        {statusLabels.map((s) => (
          <button
            ...
            className={`rounded-md px-2.5 py-1 text-xs font-medium transition-colors ${statusFilter === s
                ? 'bg-bg-active text-accent'
                : 'text-text-secondary hover:bg-bg-hover hover:text-text-primary'
              }`}
          >
        ...
        {conversations.map((conv) => {
          ...
          return (
            <button
              ...
              className={`w-full border-b border-border-subtle px-4 py-3 text-left transition-colors ${isActive ? 'bg-bg-active' : 'hover:bg-bg-hover'
                }`}
            >
              <div className="flex items-start justify-between gap-2">
                <span className="truncate text-sm font-medium text-text-primary">{conv.subject}</span>
                {unread > 0 && (
              ...
              <div className="mt-1 flex items-center gap-2">
                <span className={`inline-block h-1.5 w-1.5 rounded-full ${conv.status === 'waiting' ? 'bg-warning' :
