  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-xl border border-border-card bg-bg-card">
      ...
      <div className="flex-1 overflow-auto p-4">
        {renderWidgetBody(widget.type, data)}
      </div>
    </div>
  );
}

function renderWidgetBody(type: string, data: unknown) {
  switch (type) {
    case 'kpi': {
    ...
    default:
      return <p className="text-sm text-text-secondary">Type de widget inconnu : {type}</p>;
  }
}
